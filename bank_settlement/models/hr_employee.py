...
class HrEmployee(models.Model):
    ...
    def unlink(self):
        for employee in self:
            ...
            # أسطر استحقاق "الدفعة المقدمة" ليست ضمن النماذج الخمسة أعلاه
            # (نموذج مساعد وليس شاشة سداد مستقلة)، لكن حذف الموظف يقطع
            # التوزيع التحليلي لكل قيد لم يُرحَّل بعد - فتُفحص هنا صراحة
            # برسالة مفهومة، فوق ondelete='restrict' على مستوى قاعدة
            # البيانات (انظر prepaid_schedule.py).
            if self.env['bank.settlement.prepaid.line'].sudo().search_count(
                [('employee_id', '=', employee.id)]
            ):
                raise UserError(_(
                    'لا يمكن حذف الموظف "%s" نهائياً - له جدول/جداول '
                    'استحقاق "دفعة مقدمة" مرتبطة به. استخدم "أرشفة" '
                    'بدلاً من الحذف.'
                ) % employee.name)
        return super().unlink()
