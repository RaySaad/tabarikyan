...
VEHICLE_CHANGE_TYPES = [
    ...
    ('breakdown', 'عطل'),
    ('plate', 'لوحة'),
    # يُنشأ تلقائياً عند اعتماد إنهاء خدمة موظف (سحب المركبة بلا بديل) -
    # لا يمر بمرحلة مدير الصيانة (لا علاقة لها) وبلا مرفقات إجبارية،
    # لكنه يمر بمرحلة الاستلام كاملة: المركبة لا تعود "متاحة" قبل
    # استلامها فعلياً وتسجيل عدادها وحالتها.
    ('exit', 'خروج مندوب'),
]
